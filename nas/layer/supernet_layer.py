...
class SupernetLayer(nn.Module):
    ...
    def forward(self, batch):
        ...
        if len(self.local_gnn_types) >= 1:
            ...
            # function 2
            # average_h_local = torch.cat(h_local_list, dim=-1)
            # average_h_local = self.local_gnn_mlp(average_h_local)
        ...
        if len(self.global_model_types) >= 1:
            ...
            # function 2
            # average_h_attn = torch.cat(h_attn_list, dim=-1)
            # average_h_attn = self.global_model_mlp(average_h_attn)
